/*
 * walkGPS stage 2 : determine the RX buffer size of SoftwareSerial, by interrupting
 * the receiving process for a increasing time until problems arise
 * Conclusion : no problem observed up to 65 ms of interruption, so the announced capacity
 * of 64 bytes is confirmed.
 * 
...
switch ( step ) {
  ...
  case 100 : if ( ( c >= '0' ) && ( c <= '9' ) ) {
                ...
                if  ( sentence == 1 ) {
                    switch ( ifield ) {
                      case 1 : utime = val;
                            if ( verb2 ) { Serial.print(" utime="); Serial.print(utime); x = utime % 10; }
                            break;
                      case 7 : sats = val;
                ...
                if ( ( verb1 ) && ( sentence == 2 ) ) Serial.println("-----");  // burst separation
             }
      break;
  } // switch step
